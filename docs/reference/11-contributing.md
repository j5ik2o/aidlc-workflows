...
harness/<name>/      # Per-harness authored surfaces; claude/, kiro/, kiro-ide/, codex/, opencode/, copilot/, kimi/
...
dist/<harness>/      # GENERATED + ignored: dist/claude/, dist/kiro/, dist/kiro-ide/, dist/codex/, dist/opencode/, dist/copilot/, dist/kimi/ — never hand-edit or commit
...
The lifecycle surfaces that validate a framework version accept the fork suffix as well as a plain upstream release: the projection stamp and distribution descriptor, the per-user release directory names, `aidlc config --pin`, the Windows launcher shims, and `install.sh` / `install.ps1`. Upstream accepts only `X.Y.Z` there, so this widening is a fork change that must be carried through every upstream sync — without it a fork install reports `no installed project harness` on every `aidlc doctor` and `aidlc config` surface.
