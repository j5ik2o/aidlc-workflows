...
- **A plugin IS a host plugin.** The packager emits real `.claude-plugin/` / `.codex-plugin/` / Copilot `.plugin/` / `.kiro-plugin/` / `.opencode-plugin/` / `.cursor-plugin/` / `.kimi-code-plugin/` manifests, installed through the host's native commands. AIDLC runs no distribution infrastructure.
