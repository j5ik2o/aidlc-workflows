...
| `balanced` | architecture-reviewer, product-lead | Reviewer-shaped work -- novel input against explicit criteria. Mid-size model at reduced effort on Claude Code and opencode; Codex inherits the session model at medium effort (Kiro, Cursor, and Copilot inherit the session model like all tiers) |
| `templated` | delivery, operations, pipeline-deploy | Dominantly pattern-following output; methodology already in knowledge (delivery plans, CI/CD YAML, runbooks). Mid-size model at reduced effort on Claude Code and opencode; Codex inherits the session model at medium effort (Kiro, Cursor, and Copilot inherit the session model like all tiers) |
...
| `balanced` | `model: sonnet`, `effort: medium` | model omitted (inherits session), `model_reasoning_effort = "medium"` | field OMITTED (see below) | no tier entry | `model: amazon-bedrock/global.anthropic.claude-sonnet-4-6`, `variant: medium` | omitted (inherits session model) | `model:` OMITTED (see below) |
| `templated` | `model: sonnet`, `effort: medium` | model omitted (inherits session), `model_reasoning_effort = "medium"` | field OMITTED (see below) | no tier entry | `model: amazon-bedrock/global.anthropic.claude-sonnet-4-6`, `variant: medium` | omitted (inherits session model) | `model:` OMITTED (see below) |

Key facts behind the table:

- **Omission is the inherit mechanism.** On Claude Code an agent .md with no `effort:` key inherits the session effort, and a pinned `effort:` overrides the session in BOTH directions (a pin is a cap, not a floor) -- so absence is the contract for judgment alone; `balanced` and `templated` both pin `effort: medium`. On Codex every role TOML omits `model` and inherits the user-selected session model (verified live on codex-cli 0.139.0 and 0.142.5; the current doctor-enforced minimum is 0.145.0 for immediate compact-session reload). On Kiro CLI the agent-v1 schema documents the absent-`"model"` fallback: "If not specified, uses the default model"; Kiro IDE likewise inherits when its agent `.md` omits `model:`.
