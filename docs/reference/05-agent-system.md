...
| `balanced` | architecture-reviewer, product-lead | Reviewer-shaped work -- novel input against explicit criteria. The measured baseline pins a mid-size model at medium effort on Claude Code and opencode; Codex inherits the session model at medium effort |
...
| `balanced` | `model: sonnet`, `effort: medium` | no `model` key (inherits the session model), `model_reasoning_effort = "medium"` | field OMITTED (see below) | no tier entry | `model: amazon-bedrock/global.anthropic.claude-sonnet-4-6`, `variant: medium` | omitted (inherits session model) | `model:` OMITTED (see below) |
...
- Across the full 14-agent roster, nine agents carry the `judgment` tier, three carry the inheriting `templated` tier, and only the two `balanced` reviewers step down on Claude Code and opencode (on Codex they inherit the session model at medium effort). On Kiro, Cursor, and Copilot all tiers inherit the session model and effort. The matrix above covers the 11 domain-expert agents.
