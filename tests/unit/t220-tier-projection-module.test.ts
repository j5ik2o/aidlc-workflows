...
describe("t220 shipped projection bytes (codex TOML, kiro JSON + md)", () => {
  ...
  test("codex TOMLs: all tiers inherit model; balanced and templated pin effort", () => {
    ...
    expect(/^model\s*=/m.test(lead)).toBe(false);
    ...
    expect(/^model\s*=/m.test(delivery)).toBe(false);
