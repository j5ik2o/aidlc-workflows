...
function freshKimiProject(): { proj: string; home: string } {
  ...
  const home = mkdtempSync(join(tmpdir(), "t339-kimi-home-"));
  created.push(home);
  return { proj, home };
}
...
function runDoctor(proj: string, home: string): DoctorResult {
  // --verbose is required: the 2.8.x report collapses every passing row into a
  // "N checks passed" summary, and these wiring rows are advisory (pass: true).
  const res = spawnSync(BUN, [UTIL, "doctor", "--project-dir", proj, "--verbose"], {
    encoding: "utf-8",
    env: {
      ...process.env,
