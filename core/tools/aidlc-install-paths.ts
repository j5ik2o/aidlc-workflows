...
// A framework version is a strict semver upstream base, optionally carrying
// this fork's `-j5ik2o.<revision>` suffix (see AGENTS.md § Fork and Changelog
// Policy). Every surface that validates a framework version — projection
// stamps, release directory names, project pins — accepts both spellings, so a
// fork release is a first-class installable version rather than a rejected one.
...
export function requireVersion(value: string): string {
  if (!STRICT_SEMVER.test(value)) {
    throw new Error(
      `invalid version "${value}"; expected strict semver (for example 2.5.0) ` +
        `or a fork revision (for example 2.5.0-j5ik2o.1)`,
    );
  }
  return value;
}
