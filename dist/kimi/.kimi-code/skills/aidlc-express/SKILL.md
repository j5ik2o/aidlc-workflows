...
description: >
  ...
  detection. Lightest run: requirements to deploy, no design pass, no reviewers. Packaging over `/skill:aidlc --scope express`, which works
  without this skill.
...
1. `directive = bun .kimi-code/tools/aidlc-orchestrate.ts next --scope express $ARGUMENTS`
2. Before acting on each directive, read
   `.kimi-code/aidlc-common/protocols/stage-protocol.md` once per session,
   then read every
   `.kimi-code/aidlc-common/protocols/stage-protocol-<module>.md` named by
...
3. `bun .kimi-code/tools/aidlc-orchestrate.ts report --stage <directive.stage> --result <outcome> [--user-input "<text>"]` when the directive names a stage; omit `--stage` only for non-stage report round-trips.
...
- **Default to CONTINUATION.** Treat the input as new-work ONLY when it clearly
  ...
  (`bun .kimi-code/tools/aidlc-utility.ts intent --json` gives its `slug` and
...
- **On CONFIRM**, re-run `next` with `--new-intent`, the confirmed scope, and the
  ...
  ```bash
  bun .kimi-code/tools/aidlc-orchestrate.ts next --new-intent --scope <the confirmed scope> "<the new-work description>"
  ```
