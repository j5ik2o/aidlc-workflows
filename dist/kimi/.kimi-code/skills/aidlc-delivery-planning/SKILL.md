...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts next --stage delivery-planning --single
   ```
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts report --single --stage delivery-planning --result completed
   ```
