...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts next --stage environment-provisioning --single
   ```
...
2. Before acting on the directive, read
   `.kimi-code/aidlc-common/protocols/stage-protocol.md`. Then read every
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts report --single --stage environment-provisioning --result completed
   ```
