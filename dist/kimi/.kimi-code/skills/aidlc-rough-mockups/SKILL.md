...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts next --stage rough-mockups --single
   ```
...
3. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/aidlc-orchestrate.ts report --single --stage rough-mockups --result completed
   ```
