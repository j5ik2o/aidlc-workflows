...
`.kimi-code/aidlc-common/protocols/stage-protocol-construction.md`.
...
.kimi-code/tools/aidlc-utility.ts scope-table` when you need those
...
- **Consolidated summary before generation**: After all questions have been
  ...
  `bun .kimi-code/tools/aidlc-review-brief.ts summary --stage "<directive.stage>" --questions-file "<questions-path>"`;
  ...
  `bun .kimi-code/tools/aidlc-log.ts decision --stage <slug>
  ...
  `bun .kimi-code/tools/aidlc-log.ts answer --stage <slug>
  --checkpoint summary-confirmation --questions-file "<questions-path>"
...
1. BEFORE presenting the approval question: `bun .kimi-code/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval`.
...
1. BEFORE presenting the question: `bun .kimi-code/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .kimi-code/tools/aidlc-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
State and audit updates use the CLI tools in `.kimi-code/tools/`. These tools handle atomic read-modify-write, timestamp generation, and audit formatting internally. Do NOT use Edit or Write for these updates — those tools show diffs that create visual noise.

**CWD drift warning**: If a stage runs `cd` in Bash (e.g., `cd todo-app/server && npm install`), subsequent `bun .kimi-code/tools/...` calls using relative paths will fail with "Module not found". Always use absolute paths to the tools directory for tool calls (on Claude Code, `$CLAUDE_PROJECT_DIR/.claude/tools/`), or run `cd` commands in subshells: `(cd subdir && npm install)`.
...
```bash
bun .kimi-code/tools/aidlc-orchestrate.ts report \
...
```bash
bun .kimi-code/tools/aidlc-state.ts lookup phase-of SLUG          # → phase name
bun .kimi-code/tools/aidlc-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .kimi-code/tools/aidlc-state.ts lookup agent-for SLUG          # → lead agent name
bun .kimi-code/tools/aidlc-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
2. `.kimi-code/knowledge/aidlc-shared/` — shared methodology principles
3. `.kimi-code/knowledge/[agent-name]/` — agent-specific methodology
...
`.kimi-code/aidlc-common/protocols/stage-protocol-ensemble.md`.
...
.kimi-code/tools/aidlc-utility.ts scope-table` for the current
...
| **Scope** | Controls which stages execute and at what depth. Eleven built-in scopes, one file per scope under `.kimi-code/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, classic, workshop, express. Custom scopes can be added without editing this file. |
...
`.kimi-code/aidlc-common/protocols/stage-protocol-reviewer.md`.
...
- `.kimi-code/sensors/aidlc-<id>.md` — for verification checks. A project-tier manifest with a `matches:` capability glob, bound to the originating stage by appending its id to that stage's `sensors:` frontmatter list.
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .kimi-code/tools/aidlc-learnings.ts surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file — `{ stage_slug, space, intent, selections[] }`, where `space`/`intent` are carried verbatim from step 2's surface output — and call:
   ```bash
   bun .kimi-code/tools/aidlc-learnings.ts persist --slug <stage-slug> --selections-json <path>
   ```
   ...
   - **Sensor** → scaffolds a project-tier `<project>/.kimi-code/sensors/aidlc-<id>.md` manifest (with the user-supplied `matches:` glob) AND appends the new id to the originating stage's `sensors:` frontmatter list — both writes inside the same lock. Emits `SENSOR_PROPOSED`. The sensor binds and fires from the next workflow's compile.
...
└── Sensor (two-write install): scaffold a project-tier manifest at
    .kimi-code/sensors/aidlc-<id>.md with a matches: glob, AND append its id to
...
```bash
bun .kimi-code/tools/aidlc-state.ts reuse-artifact <stage-slug> \
