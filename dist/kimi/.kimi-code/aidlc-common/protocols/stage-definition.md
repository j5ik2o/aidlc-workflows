...
under `.kimi-code/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.kimi-code/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.kimi-code/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.kimi-code/tools/data/stage-graph.json`. Consumers continue to read the compiled
