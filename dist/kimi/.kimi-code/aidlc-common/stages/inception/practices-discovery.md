...
```bash
bun .kimi-code/tools/aidlc-state.ts practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .kimi-code/tools/aidlc-orchestrate.ts report --stage
...
```bash
bun .kimi-code/tools/aidlc-state.ts practices-promote \
