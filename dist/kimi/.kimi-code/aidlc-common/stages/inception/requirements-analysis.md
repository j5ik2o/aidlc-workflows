...
- Run the fixed command
  `bun .kimi-code/tools/aidlc-utility.ts project-description` and use its
...
- Read the selected file only through the fixed command
  `bun .kimi-code/tools/aidlc-utility.ts document-input`.
...
`bun .kimi-code/tools/aidlc-orchestrate.ts report --stage requirements-analysis --result <outcome>`.
...
`bun .kimi-code/tools/aidlc-utility.ts recompose --add user-stories`
