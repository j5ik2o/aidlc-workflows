...
`bun .kimi-code/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .kimi-code/tools/aidlc-utility.ts codekb-scope-diff --repo <repo>
```
...
```
bun .kimi-code/tools/aidlc-state.ts reuse-artifact reverse-engineering --decision keep --artifacts "<codekb-path output>" [--repo <repo>] [--single]
```
...
```
bun .kimi-code/tools/aidlc-utility.ts codekb-snapshot --repo <repo> --paths <source paths> --json
```
...
- If every repo is reused on an ordinary workflow run, report the stage as
  ...
  `bun .kimi-code/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result skipped --reason "codekb reuse: all resolved stores CURRENT, human chose reuse"`.
...
Template in `.kimi-code/knowledge/aidlc-developer-agent/re-artifacts.md`:
...
```
bun .kimi-code/tools/aidlc-log.ts link --stage reverse-engineering --link aidlc-developer-agent --artifact "<developer scan handoff path>" [--repo <repo>] [--single]
```
...
its output verbatim:
   ...
   ```
   bun .kimi-code/tools/aidlc-utility.ts codekb-scope-diff --repo <repo> --mint --paths <analyzed paths>
   ```
...
```
bun .kimi-code/tools/aidlc-utility.ts codekb-path --repo <repo>
```
...
```
bun .kimi-code/tools/aidlc-utility.ts codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft-<repo>.md
```
...
```
bun .kimi-code/tools/aidlc-utility.ts codekb-publish \
  --repo <repo> \
...
```
bun .kimi-code/tools/aidlc-log.ts link --stage reverse-engineering --link aidlc-architect-agent [--repo <repo>] [--single]
```
...
`bun .kimi-code/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result <outcome>`.
