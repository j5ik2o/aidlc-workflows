...
```bash
bun .kimi-code/tools/aidlc-testing-posture.ts render
```
...
```bash
bun .kimi-code/tools/aidlc-testing-posture.ts fingerprint --unit "<directive.unit>"
```
...
```bash
bun .kimi-code/tools/aidlc-testing-posture.ts fingerprint --stage-level
```
...
```bash
bun .kimi-code/tools/aidlc-log.ts decision --stage code-generation \
...
```bash
bun .kimi-code/tools/aidlc-log.ts answer --stage code-generation \
  --checkpoint plan-approval \
...
`bun .kimi-code/tools/aidlc-orchestrate.ts report --stage code-generation --result <outcome>`.
