...
1. The engine creates `memory.md` from
   `.kimi-code/knowledge/aidlc-shared/memory-template.md` when it emits the
   ...
   command: `mkdir -p "$(dirname "<memory_path>")" && { [ -f "<memory_path>" ] || cp ".kimi-code/knowledge/aidlc-shared/memory-template.md" "<memory_path>"; }`.
...
via `bun .kimi-code/tools/aidlc-state.ts practices-event --type override` before
