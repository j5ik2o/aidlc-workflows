...
- **bun**: Required for the CLI tools and hook scripts (tracking progress, writing the decision log, deciding what runs next). Install via \`curl -fsSL https://bun.sh/install | bash\`. The \`[[hooks]]\` commands run \`bun\` from the environment Kimi Code itself was launched with — if \`bun\` is not on that PATH (e.g. a GUI-launched terminal), launch \`kimi\` from a shell where \`bun\` resolves, export PATH in the shell that starts \`kimi\`, or edit \`.kimi-code/hooks.snippet.toml\`'s commands to an absolute bun path (e.g. \`~/.bun/bin/bun\`).
- **Hooks (manual, one-time)**: Kimi Code registers hooks only in the USER-level \`~/.kimi-code/config.toml\` — there is no project-level hook config. This install ships \`.kimi-code/hooks.snippet.toml\`; append its contents to your \`~/.kimi-code/config.toml\` and restart Kimi Code. The snippet's commands are project-relative (the hook cwd is the session's project directory) and fail-open in projects without the AI-DLC install. **Trust boundary**: because the registration is user-global and project-relative, every project you open becomes eligible to run whatever sits at \`.kimi-code/hooks/aidlc-kimi-adapter.ts\` in THAT project — append the snippet only if you work exclusively in repositories you trust. For untrusted repos, copy the whole \`.kimi-code/hooks/\` directory to a user-owned absolute path (e.g. \`~/.kimi-code/aidlc/hooks/\`) and edit the snippet's commands to that absolute path; keep the copy in step with your projects' AI-DLC version. \`/skill:aidlc --doctor\` verifies the wiring.`,
